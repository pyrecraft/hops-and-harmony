...
func _ready():
	...
	transition_duration = Globals.day_night.transition_duration if Globals.day_night else state_transition_duration
...
#	speed = (path.get_baked_points().size() / Globals.day_night.day_duration) * 60 # Match speed with day duration
	...
	baked_points_pos = midnight_step + (hour_step * Globals.day_night.day_start_hour)
