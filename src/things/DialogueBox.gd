...
export var font_height = 25
export var font_width = 25
...
func _ready():
	...
	set_text('Do you find it hard to do interesting things?')
...
func update_dialogue_box(text, font_height, font_width):
	...
	$RichTextLabel.margin_top = -top_down_offset
	$RichTextLabel.margin_bottom = top_down_offset
...
func _draw():
	pass
#	draw_rect(Rect2(Vector2(-left_right_offset, -top_down_offset), \
...
func get_dialogue_box_dimensions(text, f_height, f_width):
	...
	return get_most_squared_dimensions(nums_array, num_sum, max_num, f_height, f_width)

func get_most_squared_dimensions(nums_array, num_sum, max_num, f_height, f_width):
	var half_split_dimensions = get_squared_vector(1.0, nums_array, num_sum, max_num, f_height, f_width)
	var half_split_squaredness = abs(1.0 - (half_split_dimensions.y / half_split_dimensions.x))
	var more_than_half_split_dimensions = get_squared_vector(5.0, nums_array, num_sum, max_num, f_height, f_width)
	var more_than_half_split_squaredness = abs(1.0 - (more_than_half_split_dimensions.y / more_than_half_split_dimensions.x))
	var even_more_than_half_split_dimensions = get_squared_vector(10.0, nums_array, num_sum, max_num, f_height, f_width)
...
func get_squared_vector(multiplier, nums, sum, max_num, f_height, f_width):
